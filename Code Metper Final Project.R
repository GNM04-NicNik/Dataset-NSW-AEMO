...
# Model Kedua
model2<-arima(data_timeseries, order=c(0,1,2), seasonal=list(order = c(1, 2, 1), period = 12))
summary(model2)
coef(model2)
...
# Model Ketiga
model3<-arima(data_timeseries, order=c(0,2,3), seasonal=list(order = c(1, 1, 1), period = 12))
summary(model3)
coef(model3)
...
lines(density(residuals(model3)),col = 'red', lwd = 2)
#Karena semua diagnostik dari model menunjukkan hasil yang baik dan tidak terdapat asumsi yang ditolak
#Maka Model terbaik dalam memodelkan data adalah SARIMA(0,1,3)(1,2,1)[12]
...
ggtsdisplay(model_io)
# Perbandingan Nilai Forecast Model 1, 2 , dan 3
abs(nilai.asli-values)
abs(nilai.asli-values2)
abs(nilai.asli-values3)

#Walau hasil dari model 2 baik, tetapi asumsi tidak terpenuhi semua
#Maka Model terbaik dalam memodelkan data adalah SARIMA(0,2,3)(1,1,1)[12]
